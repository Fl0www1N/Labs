...
Для вывода нам потребуется 3 целочисленных переменных. `m` - максимальная длина подпоследовательности, это натуральная величина. `indo` - индекс элемента, с которого мы начинаем последовательность. `indвo` - индекс элемента, который мы удаляем (он может быть равен 0, что означает, что никакой элемент удалять не нужно).
...
3. **Сравнение чисел, если правое больше:**
   Программа сравнивает два рядом стоящих числаю
   Если правое больше:
      **a)**
...
5. **Вывод результата:**  
   На экран выводится поочередно максимальная длинна подпоследовательности, индекс первого элемента подпоследовательности и индекс удаленного элемента из подпоследовательности.
    ...
    public static void main(String[] args) {
        ...
        for (int i = 1; i<n; i++){
            ...
            if (a[indz] < a[i]){ // Сравниваем соседние числа
                ...
                else{ // Иначе (числа все еще подходят) увеличиваем длину, а левый индекс становится правым, чтобы проскачить удаленный элемент
                    c++;
                    indz=i;
                }
            }
